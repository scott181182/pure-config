...
var Configuration = (function () {
    ...
    Configuration.prototype.getPath = function (key) { return this.get(key); };
    ...
    Configuration.prototype.parseBlock = function (lines, root, parent) {
        ...
        lines.forEach(function (line, lineNum) {
            ...
            if (_.startsWith(line.trim(), "%include")) {
                return _.merge(root, new Configuration(_this.directory + "/" + line.substr(line.indexOf(" ") + 1)).object);
            }
            var newKey;
            if (r_nest.test(line) && lines[lineNum + 1] && lines[lineNum + 1].search(/\S/) > line.search(/\S/)) {
                ...
                if (!lines[lineNum + 1]
                    ...
                    if (blockEnd === -1) {
                        return console.error("Could not find closing bracket to list:\n" + line + "\n" + lines[lineNum + 1]);
                    }
                    skipTo = blockEnd + 1;
